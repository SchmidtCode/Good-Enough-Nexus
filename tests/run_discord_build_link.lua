...
local f=assert(io.open('core/AccountBuildWorkspace.lua','r')); local workspace=f:read('*a'); f:close()
assert(workspace:find('discord%.com/channels/',1,false), 'Discord message link validation missing')
assert(workspace:find('BroadcastBuildSummary',1,true), 'link save must broadcast updated build summary')
