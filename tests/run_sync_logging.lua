...
assert(text:find("%-%- DPS sync %-%-"), "tab missing DPS sync summary")
assert(text:find("builds in my library"), "tab missing library summary")
Sync.HandleIncoming(string.rep("X", 256), "Bob")
text = LogText()
assert(text:find("too long/control/type%s+: 1 / 0 / 0"),
    "tab missing classified wire-envelope rejection counts")
print("Sync tab includes connection, DPS, wire, counters, and library summary -- OK")
