...
assert(completed, "fair responder did not finish before pending-work expiry")
...
assert((Sync.Stats().overlaySent or 0) == 12,
    "current/compatibility delta responses duplicated or omitted admission")
assert(response.buildSerializations == 13
    and response.buildAdmissions == 13,
    "queue-full retry repeated build serialization or missed a payload")
assert(counts.json >= 13 and counts.base64 == 13,
    "response payloads were encoded more than once per request candidate")
...
for _, message in ipairs(H.sentChatMessages) do
    ...
    if not wire:find("^WLBC|") and not wire:find("^WLLC|") then
        bulkSent = bulkSent + 1
    end
end
...
assert(Sync.WorkState().sending + bulkSent
    ...
    "saturation changed, dropped, or duplicated admitted bulk queue data")
