...
- Baseline reconciliation is explicit rather than regenerated: the four remaining reviewed findings are owned by `tools/Invoke-QualityGate.ps1` and `tools/Test-ReleasePolicy.ps1`; eight Stage 41 helper findings were fixed with precise output types and singular function names instead of being accepted.
- Synthetic fixtures PASS for exact inheritance, line movement, disappearance/improvement, owner move, same-rule message replacement, identical duplicates, and stale entries. Current PSScriptAnalyzer PASS: 0 blocking, 4 advisory, 4 inherited, 0 new, 0 improvements.
